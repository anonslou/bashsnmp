...
# значения, превышающие пороговые подсвечиваются желтым и красным, слишком слабое потребление ресурсов - зеленым, недоступные хосты - пустые строки 
# скрипт из коробки работает с linux, windows и ESXi в конфиг файле надо через пробел указать snmp v2 комьюнити на чтение и доменное имя хоста: одна строчка - один сервер
...
do
	...
	# вычисляем количество ядер, а чтобы оптимизировать bulk запрос считаем, что максимум у хоста может быть 32 ядра
	...
	# к сожалению, могут быть необходимы правки под наблюдаемые системы...
